...
while True:
    ...
    for i, driver in enumerate(drivers):
        ...
        if prev_driver and driver["position"] < prev_driver["position"]:
            ...
            # Get the current datetime and reformat it to be file name friendly. Include time down to hundredths of a second
            now = datetime.datetime.now()
            now = now.strftime("%Y-%m-%d_%H-%M-%S-%f")[:-3]

            elevenlabs.save(audio, f"commentary_{now}.wav")

            # Wait half a second before looking for the next overtake
            time.sleep(0.5)