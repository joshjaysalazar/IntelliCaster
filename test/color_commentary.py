...
Keep your responses limited to no more than 10 words.
If asked to provide color commentary, comment on the race as a whole so far, and use 2 full sentences instead.
{chat_history}
Human: {question}
AI:
"""
...
while True:
    # Every now and then, provide color commentary based on what's happened so far
    # Only do so if 5 overtakes have happened so far
    if random.random() < 0.01 and overtakes > 5:
        output = llm_chain.run("Provide color commentary")
        print(output)
